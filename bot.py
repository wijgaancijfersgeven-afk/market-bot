...
db.init_db()
handle_admin_text, is_admin = register_admin_handlers(bot, sessions, ADMIN_IDS)
...
def check_membership(user_id: int) -> bool:
    ...
    try:
        ...
        return True  # Kanal bulunamadıysa engelleme
...
def require_membership(message_or_call):
    ...
    uid = (message_or_call.from_user.id
           ...
           else message_or_call.from_user.id)
    ...
    bot.send_message(chat_id,
        f"⚠️ *Devam etmek için kanala katılman gerekiyor!*\n\n"
        f"📢 Kanal: *{channel}*\n\n"
        f"Katıldıktan sonra aşağıdaki butona bas 👇",
        parse_mode="Markdown",
        reply_markup=kb.join_channel_inline(channel))
    return False
...
def start_handler(message):
    ...
    bot.send_message(message.chat.id,
        ...
        f"_Türkiye'nin güvenilir dijital ürün marketi_\n\n"
...
def text_handler(message):
    ...
    # Admin akışı — kanal kontrolü yok
    if is_admin(uid) and (
        state.startswith(("add_", "complete_", "reply_", "set_")) or
        state == "admin" or
        text in ["📊 İstatistikler", "📦 Bekleyen Siparişler", "➕ Kategori Ekle",
                 "➕ Ürün Ekle", "🎟️ Kupon Oluştur", "🎁 Çekiliş Başlat",
                 "📢 Kanal Ayarla", "🆘 Destek Talepleri", "❌ İptal",
                 "🗑 Kanalı Kaldır", "✅ Evet", "❌ Hayır"]
    ):
        if handle_admin_text(message, sess):
            return

    # Kullanıcı işlemleri — kanal kontrolü var
    if text != "🏠 Ana Menü":
        if not require_membership(message):
            return
    ...
    if text in ["👑 VIP Mağaza", "👑 VIP Ol"]:
        if not user["is_vip"]:
            ...
            bot.send_message(message.chat.id,
                ...
                f"2️⃣ *500 Puan* harca ─ Toplam harcama: *{user['total_spent']} Puan*\n\n"
                f"✨ *VIP Avantajları:*\n"
    ...
    if text == "🔄 Puan Transferi":
        ...
        bot.send_message(message.chat.id,
            ...
            f"_(ID'yi @userinfobot'tan öğrenebilirsin)_",
...
def callback_handler(call):
    ...
    # Kanal kontrolü — diğer callbackler için
    if not check_membership(uid):
        channel = db.get_setting("required_channel")
        bot.answer_callback_query(call.id, "⚠️ Önce kanala katılmalısınız!", show_alert=True)
    ...
    if data.startswith("join_giveaway_"):
        ...
        if joined:
            ...
            bot.edit_message_text(
                f"🎉 *Çekilişe Katıldınız!*\n\n"
                f"Kazananlar açıklandığında bildirim alacaksınız. Bol şans! 🍀",
                call.message.chat.id, call.message.message_id, parse_mode="Markdown")
        else:
    ...
    if data in ["lang_tr", "lang_en"]:
        lang = "Türkçe 🇹🇷" if data == "lang_tr" else "English 🇬🇧"
        bot.answer_callback_query(call.id, f"{lang} seçildi!")
        bot.edit_message_text(f"🌍 *Dil: {lang}*",
            call.message.chat.id, call.message.message_id, parse_mode="Markdown")
        return
